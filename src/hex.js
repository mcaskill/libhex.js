...
HEX.to_direction = function (c) {
	if (c.length !== 1 || c < 'A' || c > 'F') throw HEX.invalid_argument("to_direction: " + c);
	return HEX[c];
}
HEX.to_char = function (d) {
	if (d < 0 || d > HEX.DIRECTIONS.length) throw HEX.invalid_argument("to_char: " + d);
...
HEX.add = function (d, i) {
	if (typeof d === 'string') d = HEX.to_direction(d);
	d = (d + i) % HEX.DIRECTIONS.length;
	while (d < 0)
	d += HEX.DIRECTIONS.length;
	return d;
}
HEX.sub = function (d, i) {
	return HEX.add(d, -i);
}
HEX.inc = function (d) {
	return HEX.add(d, 1);
...
HEX.Point = function (a, b) {
	if (typeof a === 'string') {
		var piece = a.split(',');
		if (piece.length !== 2) throw HEX.invalid_argument(a);
...
HEX.Point.prototype = {
	...
	add: function (p) {
		return new HEX.Point(this.x + p.x, this.y + p.y);
	},
	sub: function (p) {
		return new HEX.Point(this.x - p.x, this.y - p.y);
	},
	mul: function (v) {
		return new HEX.Point(this.x * v, this.y * v);
	},
	div: function (v) {
		return new HEX.Point(this.x / v, this.y / v);
	},
...
HEX.grid = {
	...
	area: function (str) {
		...
		var result = [];
		var pos = str.search(/[>:]/);
		if (pos < 0) throw HEX.invalid_argument(str + ' [1]');
	...
	path: function (str) {
		...
		if (colon <= 0 || (colon + 1) >= str.length) throw HEX.invalid_argument(s);
...
HEX.Edge.prototype = {
	...
	complement: function () {
		...
		if (adjacent_hex) return adjacent_hex.edge(HEX.add(this.direction, 3));
		else return null;
	},
	...
	is_next: function (v) {
		...
		if (this.hex.valueOf() === v.hex.valueOf()) {
			return (HEX.add(this.direction, 1) === v.direction || HEX.sub(this.direction, 1) === v.direction);
		} else {
			var vv = v.valueOf();
			return (vv === this.next_out(true).valueOf() || vv === this.next_out(false).valueOf());
		}
	},
	...
	next_out: function (clockwise) {
		...
		if (c) return c.hex.edge(HEX.sub(this.direction, one));
		else return null;
	},
	...
	_corner_offset: function (p, d, bias) {
		...
		if (bias) return p.offset(dx * (1.0 + bias), dy * (1.0 + bias));
		else return p.offset(dx, dy);
	},
...
HEX.Area = function (hexset) {
	if (!(hexset instanceof Array)) throw HEX.invalid_argument(hexset);
	this.hexes = hexset;
}
...
HEX.Area.prototype = {
	...
	contains: function (h) {
		return HEX.set_contains(this.hexes, h);
	},
	...
	boundary: function () {
		...
		// Find an edge.
		var i = 0;
		while (i <= i0 && !this.contains(HEX.grid.hex(i, j0)))++i;
		var result = []; // edges
		result.push(HEX.grid.hex(i, j0).edge(HEX.D));
		// Follow the edge round.
		while (true) {
			var e = result[result.length - 1].next_out();
			if (!e || !this.contains(e.hex)) e = result[result.length - 1].next_in();
			if (e.valueOf() === result[0].valueOf()) break;
			result.push(e);
		}
	...
	toString: function (origin) {
		...
		for (var p = 0, len = paths.length; p < len; ++p) {
			if (paths[p].hexes[0].valueOf() !== origin.valueOf()) result += ">" + HEX.steps(origin, paths[p].hexes[0]);
	...
	if (a instanceof HEX.Hex && typeof b === 'string') {
		...
		while (cur < steps.length && steps.charAt(cur) !== '?') {
			...
			do {
				...
				if (next) this.hexes.push(next);
				else if (steps.charAt(steps.length - 1) === '?' || repeat) return; // bail out instead of throwing
				else throw HEX.out_of_range('path:' + steps);
			} while (repeat);
		}
	} else throw HEX.invalid_argument('HEX.Path()');
}

HEX.Path.prototype = {

	to_area: function () {
		return new HEX.Area(this.hexes);
	},
	length: function () {
		return this.hexes.length;
	},
	...
	steps: function () {
		...
		var curr = 0;
		for (var h = 0, len = this.hexes.length; h < len; ++h) {
			if (curr) result += HEX.steps(curr, this.hexes[h]);
			curr = this.hexes[h];
		}
		return result;
	},
...
HEX.Boundary.prototype = {
	...
	is_closed: function () {
		return (this.edges.length > 2 && this.edges[0].is_next(this.edges[this.edges.length - 1]));
	},
	...
	is_container: function () {
		if (this.is_closed()) {
			try {
				...
				if (e instanceof HEX.exception && e.kind === 'out_of_range') return true;
			}
		}
		return false;
	},
	...
	complement: function () {
		...
		for (var e = 0, len = this.edges.length; e < len; ++e) {
			...
			if (c) result.push(c);
			else throw HEX.out_of_range("Boundary complement out of range.");
		}
		return new HEX.Boundary(result);
	},
	...
	clockwise: function () {
		...
		if (this.edges.length > 1) {
			var e0 = this.edges[0];
			var e1 = this.edges[1];
			return (e0.valueOf() === e1.next_in().valueOf() || e0.valueOf() === e1.next_out().valueOf());
		}
		return false;
	},
	...
	stroke: function (bias) {
		...
		if (bias) {
			var last = null;
			for (var e = 0, len = this.edges.length; e < len; ++e) {
				if (last) result.push(last.join_point(this.edges[e], bias));
				last = this.edges[e];
			}
			if (this.is_closed()) {
...
HEX.go = function (hex, steps, distance) {
	if (typeof steps === 'number') steps = HEX.to_char(steps);
	if (steps.length > 1 || !distance) distance = 1;
	...
	for (var x = 0; x < (distance ? distance : 1); x++) {
		for (var s = 0, len = steps.length; s < len; ++s) {
			...
			if (hex.j % 2) switch (direction) // odd
			{
			case HEX.A:
				++hex.i;
				break;
			...
			case HEX.F:
				...
				break;
			default:
				throw HEX.invalid_argument('go: ' + direction);
			} else switch (direction) // even
			{
			case HEX.A:
				++hex.i;
				break;
...
HEX.set_find = function (hexset, hex) {
	// binary chop.
	var lower = 0,
		upper = hexset.length - 1;
	while (lower <= upper) {
		var pos = Math.floor((lower + upper) / 2);
		if (hexset[pos] > hex) upper = pos - 1;
		else if (hexset[pos] < hex) lower = pos + 1;
		else return pos;
	}
	return -1;
}
...
HEX.set_difference = function (a, b) {
	...
	if (!HEX.set_contains(b, a[i])) result.push(new HEX.Hex(a[i]));
	return result;
}
...
HEX._extract_connected_set = function (s) {
	...
	while (true) {
		...
		HEX.set_erase(s, h);
		if (s.length === 0) break; // optimisation - not strictly necessary.
